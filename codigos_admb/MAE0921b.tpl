...
// LEER DATOS "MAEsardina.dat"
  ...
  init_matrix matdat(1,nanos,1,15)
...
// 1. Coeficientes de variaci?n y tama?os de muestra
  ...
  init_number cvpriorq_mph
  
  init_number prior_qrecl//q reclas
  init_number prior_qpela//q pelaces
  init_number prior_qmph//q mph
  
  //log_prior_qrecl
  
  //c?lculo preliminar
  number log_prior_qrecl
  number log_prior_qpela
  number log_prior_qmph
  
  !! log_prior_qrecl = log(prior_qrecl);
  !! log_prior_qpela = log(prior_qpela);
...
// 2. Fases de selectividad 
  ...
  init_int    nSbloques1 //numero de bloques de selectividad flota
  init_vector ySbloques1(1,nSbloques1)//year de inicio bloque de selectividad flota
  
  init_int    nSbloques2//numero de bloques de selectividad RECLAS
  init_vector ySbloques2(1,nSbloques2)//year de inicio bloque de selectividad RECLAS
  
  init_int    nSbloques3//numero de bloques de selectividad PELACES
  init_vector ySbloques3(1,nSbloques3)//yearde inicio bloque de selectividad PELACES
...
// 3. Fases de capturabilidad
  ...
  init_int    nqbloques1//numero de bloques de Capturabilidad Reclas
  init_vector yqbloques1(1,nqbloques1)// year de inicio bloque de Capturabilidad Reclas
  
  init_int    nqbloques2//numero de bloques de Capturabilidad Pelaces
  init_vector yqbloques2(1,nqbloques2)//year de iniciobloque de Capturabilidad Pelaces
...
// selectividad param?trica
 init_bounded_vector A50flota(1,nSbloques1,-1,2,Fase_Sflota)  //incorporar bloque de selectividad (julio 2019)
 init_bounded_vector log_rangoflota(1,nSbloques1,-4,0,Fase_Sflota)

 init_bounded_vector A50reclas(1,nSbloques2,-1,2,Fase_Sreclas)  
 init_bounded_vector log_rangoreclas(1,nSbloques2,-4,0.6,Fase_Sreclas)

 init_bounded_vector A50pelaces(1,nSbloques3,-1,2,Fase_Spelaces)  
 init_bounded_vector log_rangopelaces(1,nSbloques3,-4,0.6,Fase_Spelaces)
...
// capturabilidades
 init_vector log_qrecl(1,nqbloques1,opt_qrecl) // reclas
 init_vector log_qpela(1,nqbloques2,opt_qpela) // pelaces
...
//##############################################################################
  ...
  // 1. SELECTIVIDAD
  //=====================================
  matrix S1(1,nSbloques1,1,nedades)
  matrix Sel_flota(1,nanos,1,nedades)
  
  matrix S2(1,nSbloques2,1,nedades)
  matrix Sel_reclas(1,nanos,1,nedades)
  
  matrix S3(1,nSbloques3,1,nedades)
...
// leo la matriz de indices
  ...
  Nm(1)=column(matdat,10);
  Nm(2)=column(matdat,11);
  Nm(3)=column(matdat,12);
  
  dtR=column(matdat,13);
  dtP=column(matdat,14);
  dtmph=column(matdat,15);
...
//===============================================================================
 ...
 for (j=1;j<=nSbloques1;j++){
  S1(j)=1/(1+exp(-log(19)*(edades-exp(A50flota(j)))/exp(log_rangoflota(j))));}

 for (i=1;i<=nanos;i++){
  for (j=1;j<=nSbloques1;j++){
    if (anos(i)>=ySbloques1(j)){
      Sel_flota(i)=S1(j);}}}
  ...
  for (j=1;j<=nSbloques3;j++){
    S3(j)=1/(1+exp(-log(19)*(edades-exp(A50pelaces(j)))/exp(log_rangopelaces(j))));}
  
  for (i=1;i<=nanos;i++){
    for (j=1;j<=nSbloques3;j++){
      if (anos(i)>=ySbloques3(j)){
        Sel_pelaces(i)=S3(j);}}}
  ...
  // Sel_reclas  = outer_prod(Unos_anos,(elem_div(Unos_edad,(1+exp(-1.0*log(19)*(edades-A50reclas)/exp(log_rangoreclas))))));
     ...
     for (j=1;j<=nSbloques2;j++){
       S2(j)=1/(1+exp(-log(19)*(edades-exp(A50reclas(j)))/exp(log_rangoreclas(j))));}
     
     for (i=1;i<=nanos;i++){
       for (j=1;j<=nSbloques2;j++){
         if (anos(i)>=ySbloques2(j)){
           Sel_reclas(i)=S2(j);}}}
...
// matrices y vectores de abundancias derivadas
  ...
  NVreclas(i)    = elem_prod(elem_prod(N(i),mfexp(-dtR(i)*Z(i))),Sel_reclas(i));// Crucero Reclas
  }
  
  for(int i=1;i<=nanos;i++){
  NVpelaces(i)    = elem_prod(elem_prod(N(i),mfexp(-dtP(i)*Z(i))),Sel_pelaces(i));// Pelaces
  }
// correcci?n por error de asignaci?n de la edad
  ...
  NMD      = elem_prod(elem_prod(N,mfexp(-dt(4)*Z)),outer_prod(Unos_anos,msex));
...
// vectores de biomasas derivadas
  ...
  Bmph     = rowsum(elem_prod(NVmph,Win));    // Bmph
 ...
 //MPH_pred      = exp(log_qmph)*Bmph;
  ...
  for (int i=1;i<=nanos;i++){
   for (int j=1;j<=nqbloques1;j++){
     if (anos(i)>=yqbloques1(j)){
       Reclas_pred(i)=mfexp(log_qrecl(j))*Breclas(i);}
    }
  }
 
  for (int i=1;i<=nanos;i++){
   for (int j=1;j<=nqbloques2;j++){
     if (anos(i)>=yqbloques2(j)){
       Pelaces_pred(i)=mfexp(log_qpela(j))*Bpelaces(i);}
    }
  }
...
// multinomial
  likeval(5)   = -sum(Nm(1)*elem_prod(pobs_f,log(ppred_f)));
  likeval(6)   = -sum(Nm(2)*elem_prod(pobs_crua,log(ppred_crua)));
  likeval(7)   = -sum(Nm(3)*elem_prod(pobs_pel,log(ppred_pel)));
// tallas del pelaces
...
// Penaliza F 1991-1992 al promedio
  likeval(10) = 1000*(square(log_Ft(2)-mean(log_Ft))+square(log_Ft(3)-mean(log_Ft)));  // S12

// Penaliza Fspr
  if(active(log_Fref)){
  likeval(11) = 1000*norm2(ratio_spr-ratio);}
 
 // q cruceros
  //likeval(10)  = 1./(2*square(cvpriorq_reclas))*square(log_qrecl);
  //likeval(11)  = 1./(2*square(cvpriorq_pelaces))*square(log_qpela);
  //likeval(12)  = 1./(2*square(cvpriorq_mph))*square(log_qmph);
  
 if(active(log_qrecl)){
   penalty+=0.5*norm2((log_qrecl-log_prior_qrecl)/cvpriorq_reclas);}
 
 if(active(log_qpela)){
   penalty+=0.5*norm2((log_qpela-log_prior_qpela)/cvpriorq_pelaces);}
 
 if(active(log_qmph)){
   penalty+=0.5*norm2((log_qmph-log_prior_qmph)/cvpriorq_mph);}
...
//----------------------------------------------------------------------------------
 ...
 report<<sqrt(suma1/cuenta1)<<" "<<sqrt(suma2/cuenta2)<<" "<<sqrt(suma4/cuenta4)<<endl;
  ...
  report << " ------------------------------------------------" << endl;
  report << "Talla a la edad & desviacion "<< endl;
  report << mu_edad << endl;

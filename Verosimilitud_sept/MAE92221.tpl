...
// parametros reclutamientos y mortalidades)
 ...
 //init_number log_Ro(opt_Ro) //probar escenarios
 init_bounded_vector log_desv_No(1,nedades-1,-10,10,opt_devNo)
 //init_vector log_desv_No(1,nedades-1,opt_devNo) //probar escenarios
 init_bounded_dev_vector log_desv_Rt(1,nanos,-10,10,opt_devR)
 //init_vector log_desv_Rt(1,nanos,opt_devR) //probar escenarios
 init_bounded_vector log_Ft(1,nanos,-6,1.6,opt_F) // log  mortalidad por pesca por flota
 //init_vector log_Ft(1,nanos,opt_F)  //probar escenarios
...
// reclutas anuales a la edad 2
  ...
  for (i=1;i<=nanos;i++)
  {
    ...
    //N(i,1) = mfexp(log_Ro+log_desv_Rt(i)-0.5*square(sigmaR)); //probar escenario sesgo negativo
  }
    Neq(1) = mfexp(log_Ro+0.5*square(sigmaR));
    //Neq(1) = mfexp(log_Ro-0.5*square(sigmaR)); // probar escenario sesgo negativo
...
//  Reclutas
  ...
  //likeval(9)   = 0.5*sum(log_desv_Rt^2); //probar escenario....revisar, tengo problemas para compilar!!!
  
// q cruceros
  likeval(10)  = 1./(2*square(cvpriorq_reclas))*square(log_qrecl); //probar escenario bajando el cvreclas a 0.3 pro ejemplo u otro en .dat
  likeval(11)  = 1./(2*square(cvpriorq_pelaces))*square(log_qpela);

// Penaliza F 1991-1992 al promedio
  likeval(12) = 1000*(square(log_Ft(2)-mean(log_Ft))+square(log_Ft(3)-mean(log_Ft)));  
  
  //Respecto a la penalización de la mortalidad por pesca
  // probar escenario eliminando penalización de F
  // incorporar escenario utilizando penalización al reclutamiento, revisar Derova y Miller 2016
...
//************************************************************************************************
  ...
  // Estimaci?n de CBA A?O BIOL?GICO sin proyectar//revisar ?ltimo a?o!!!
    ...
    Fref_r0 = exp(log_Ft(nanos));} //
    
	  Frms_r0 = Sel_flota(nanos)*Fref_r0;
    Zrms_r0 = Frms_r0+M;
    Nvult_r0= elem_prod(1.-exp(-1.*Zrms_r0),N(nanos));
    
    CTP_r0  = elem_prod(elem_div(Frms_r0,Zrms_r0),elem_prod(1.-exp(-1.*Zrms_r0),N(nanos))); //matriz se captura en número
    YTP_r0W  = elem_prod(CTP_r0,Wmed(nanos));   // matriz de captura en peso
    YTP_r0  = sum(elem_prod(CTP_r0,Wmed(nanos)));  // Captura en toneladas año biológico actual
  ...
  // descuento del descarte para CBA
    ...
    YTP_r0d1ersemR = YTP_r0d - (Desemb2doSem+Remanente); // Captura en toneladas del primer semestre (enero a junio)
	  ...
	  NMD_r0  = elem_prod(elem_prod(N(nanos),mfexp(-dt(3)*Zrms_r0)),msex); // matriz abundancia desovante en número
    BD_r0   = sum(elem_prod(NMD_r0,Win(nanos)));	// biomasa desovante en peso
    RPR_r0  = BD_r0/Brms(1); // razón Biomasa desovante actual y biomasa desovante objetivo
...
// Escenarios de reclutamiento promedio
  if(oprec==1){Np(1)=mfexp(mean(log(Reclutas(nanos-11,nanos))));} // Reclutamiento promedio desde 2008-2019 (recientes) 
  if(oprec==2){Np(1)=mfexp(log_Ro+0.5*square(sigmaR));} // Reclutamiento promedio hist?rico (hist?rico + error)
  if(oprec==3){Np(1)=mfexp(mean(log(Reclutas)));} // Reclutamiento promedio desde 1991-2019 (hist?rico)
//--------------------------------------------------------
// Escenarios de reclutamiento basados de an?lisis de quiebre (revisar todos los a?os)
  if(oprec==4){Np(1)=mfexp(mean(log(Reclutas(1,nanos-15))));} //115218; // promedio 1991-2007// 1er quiebre (inicios de la serie)
  if(oprec==5){Np(1)=mfexp(mean(log(Reclutas(nanos-14,nanos-10))));} //412683 // promedio 2008-2012 // 2do quiebre (a mitad de la serie)
  if(oprec==6){Np(1)=mfexp(mean(log(Reclutas(nanos-9,nanos))));} //188921// promedio entre 2013-2022 " 3er quiebre (al final de la serie)"
  
  //if(oprec==4){Np(1)=mean(Reclutas(1,nanos-15));} //115218; // promedio 1991-2007// 1er quiebre (inicios de la serie)
  //if(oprec==5){Np(1)=mean(Reclutas(nanos-14,nanos-10));} //412683 // promedio 2008-2012 // 2do quiebre (a mitad de la serie)
  //if(oprec==6){Np(1)=mean(Reclutas(nanos-9,nanos));} //188921// promedio entre 2013-2022 " 3er quiebre (al final de la serie)"
...
//----------------------------------------------------------------
    if(opProy==1) // Opci?n 1: 1era y 2da revisi?n (para el mismo a?o)
    {
     ...
     CBA_c0  =  (prop(1)*YTP_r0) + (prop(2)*YTP_p0(1)); // proporción x%/30 sin descontar el descarte 
     ...
     CBA_c0d = (prop(1)*YTP_r0d) + (prop(2)*YTP_p0d(1)); //  proporción x%/30 con descuento del descarte
    ...
    if(opProy==2) // Opci?n 2: CBA inicial (proyecci?n de un a?o calendario)
    {
     ...
     CBA_c0d=prop(1)*(YTP_p0(1)*Descarte)+prop(2)*(YTP_p0(2)*Descarte); 
